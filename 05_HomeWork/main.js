  ...
  constructor(data, parent) {
    ...
    <div class="card">
      ...
      <div class="card-action">
        <a href = "#info" class = "teal-text more" data-film-id = "${data.imdbID}">Details</a>
      </div>
    </div>
    `;
  }
    ...
    <div class="card horizontal">
      ...
      <div class="card-stacked">
        <div class="card-content">
          ...
          <div class="progress">
            <div class="determinate" style="width: ${data.imdbRating * 10}%"></div>
          </div>
          <p>
            Duration:${Math.trunc(+data.Runtime.split(" ")[0] / 60)}
            h : ${+data.Runtime.split(" ")[0] % 60}min
          </p>
...
buttonSearch.addEventListener("click", function () {
  main.children.length > 0 && [...main.children].forEach((e) => main.removeChild(e));

  const filmName = input.value;
  const filmType = select.value;

  if (input.value == "") return M.toast({
    html: "Input is empty"
  });
  ...
  fetch(
    ...
    .then((data) => {
      ...
      linksMore.forEach((e) =>
        e.addEventListener("click", function () {
          info.children.length > 0 && [...info.children].forEach((e) => info.removeChild(e));
          fetch(
